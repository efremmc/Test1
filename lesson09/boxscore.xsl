...
<xsl:template match="team">
   ...
   <table class="bs">
   <tr>
   <th colspan="9" class="title"><xsl:value-of select="@name" /> [<xsl:value-of select="$tpts" />] </th>
   </tr>
   <tr><th>Player</th><th>FGM-FGA</th><th>FTM-FTA</th><th>OFF-DEF-TOT</th>
       <th>ASST</th><th>ST</th><th>TO</th><th>PF</th><th>PTS</th>
   </tr>
   <xsl:apply-templates select="player" />
   <tr><td colspan="9"><hr /></td></tr>
   <tr><td class="name">Total</td>
       <td><xsl:value-of select="$tfgm" />-<xsl:value-of select="$tfga" /></td>
       <td><xsl:value-of select="$tftm" />-<xsl:value-of select="$tfta" /></td>
       <td><xsl:value-of select="$toff" />-<xsl:value-of select="$tdef" />-<xsl:value-of select="$ttotr" /></td>
       <td><xsl:value-of select="$tasst" /></td>
       <td><xsl:value-of select="$tst" /></td>
       <td><xsl:value-of select="$tto" /></td>
       <td><xsl:value-of select="$tpf" /></td>
       <td><xsl:value-of select="$tpts" /></td>
   </tr>
   </table>

   <table class="tpfg">
   <tr><td class="tpfg" width="100">Three-pt. FGs</td>
       <td class="tpfg" width="50"><xsl:value-of select="$tfgmade3" /></td>
       <td class="tpfg">
...
<xsl:template match="player">
   ...
   <tr>
   <td class="name"><xsl:value-of select="@name" /> (<xsl:value-of select="@position" />)</td>
   <td><xsl:value-of select="$fgm" />-<xsl:value-of select="$fga" /></td>
   <td><xsl:value-of select="$ftm" />-<xsl:value-of select="$fta" /></td>
   <td><xsl:value-of select="$off" />-<xsl:value-of select="$def" />-<xsl:value-of select="$totr" /></td>
   <td><xsl:value-of select="$asst" /></td>
